...
(async()=>{
 ...
 await page.selectOption('#asset','NQ_YAHOO');
 const dailyPresentation=await page.evaluate(()=>{const s=chartApp.getState(),b=DATA.daily.NQ_YAHOO.bars[s.cursor];return {validOHLC:b[6]===1,header:document.getElementById('ohlc').textContent,aria:document.getElementById('priceChart').getAttribute('aria-label'),legend:document.getElementById('candleLegend').textContent,htf:document.getElementById('htfLiquidity').textContent};});
 assert.equal(dailyPresentation.validOHLC,true);assert.match(dailyPresentation.header,/O.*H.*L.*C/);assert.match(dailyPresentation.aria,/Daily OHLC.*candlesticks/);assert.match(dailyPresentation.legend,/OHLC candles/);assert.match(dailyPresentation.htf,/HTF LIQUIDITY PROXY.*WEEKLY SOURCE.*BSL.*SSL/);pass('Daily view exposes verified OHLC candlesticks and weekly HTF liquidity output',dailyPresentation);
