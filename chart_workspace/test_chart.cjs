...
(async()=>{
 ...
 const audit=await page.evaluate(()=>{
  ...
  for(const id of Object.keys(DATA.series)){
   ...
   for(const c of [0,Math.min(32,bs.length-1),Math.floor(bs.length*.25),Math.floor(bs.length*.5),bs.length-1]){
    ...
    for(const l of sc.structure.filter(l=>l.subtype!=='period'))ok(l.confirmed<=c,`${id}: unconfirmed swing/equal level`);
   }
  }
 ...
 await page.selectOption('#asset','NQ_YAHOO');
 const dailyPresentation=await page.evaluate(()=>{const s=chartApp.getState(),b=DATA.daily.NQ_YAHOO.bars[s.cursor],structure=chartApp.getScene().structure;return {cursor:s.cursor,validOHLC:b[6]===1,header:document.getElementById('ohlc').textContent,aria:document.getElementById('priceChart').getAttribute('aria-label'),legend:document.getElementById('candleLegend').textContent,htf:document.getElementById('htfLiquidity').textContent,structureText:document.getElementById('structureOutput').textContent,structure:structure.map(l=>({id:l.id,confirmed:l.confirmed}))};});
 assert.equal(dailyPresentation.validOHLC,true);assert.match(dailyPresentation.header,/O.*H.*L.*C/);assert.match(dailyPresentation.aria,/Daily OHLC.*candlesticks/);assert.match(dailyPresentation.legend,/OHLC candles/);assert.match(dailyPresentation.htf,/HTF LIQUIDITY PROXY.*WEEKLY SOURCE.*BSL.*SSL/);assert.match(dailyPresentation.structureText,/Previous week high.*Previous week low.*Previous month high.*Previous month low.*Confirmed swing high.*Confirmed swing low.*Relative equal (highs|lows)/);assert.equal(dailyPresentation.structure.filter(l=>l.confirmed!==undefined).every(l=>l.confirmed<=dailyPresentation.cursor),true);pass('Daily view exposes OHLC candles, HTF periods, confirmed swings and relative equal highs/lows',dailyPresentation);
 await page.selectOption('#structureLayer','periods');assert.equal(await page.evaluate(()=>chartApp.getScene().structure.every(l=>l.subtype==='period')),true);await page.selectOption('#structureLayer','swings');assert.equal(await page.evaluate(()=>chartApp.getScene().structure.every(l=>l.subtype!=='period')),true);await page.selectOption('#structureLayer','off');assert.equal(await page.evaluate(()=>chartApp.getScene().structure.length),0);await page.selectOption('#structureLayer','all');pass('Structure layer control separates period, swing/equal and off views');
 await page.click('#sourceBtn');assert.match(await page.locator('#drawerBody').innerText(),/FROZEN RESEARCH INPUTS.*DAILY/s);assert.match(await page.locator('#drawerBody').innerText(),/computed from saved daily bars/i);await page.click('#closeDrawer');await page.click('#evidenceBtn');assert.match(await page.locator('#drawerBody').innerText(),/visible chart is daily/i);await page.click('#closeDrawer');
